// confWeb.h - configuration page for WLAN access using AsyncWebServer
//
// 2023-01-21 mh
// - derived from IotWebConf.h
...
class StandardWebRequestWrapper
{
public:
  StandardWebRequestWrapper(AsyncWebServerRequest* request) { this->_request = request; };

  const String hostHeader() const  { return this->_request->host(); }; 
  IPAddress localIP()  { return this->_request->client()->localIP(); };
  uint16_t localPort()  { return this->_request->client()->localPort(); };
  const String uri() const { return this->_request->url(); };
  bool authenticate(const char * username, const char * password) 
    { return this->_request->authenticate(username, password); };
  void requestAuthentication() 
    { this->_request->requestAuthentication(); };
  bool hasArg(const String& name)  { return (this->_request->hasArg(name.c_str())); };
  String arg(const String name)  { return this->_request->arg(name); };
//  void sendHeader(const String& name, const String& value, bool first = false) 
//    { this->_request->sendHeader(name, value, first); };
...
//    { this->_request->setContentLength(contentLength); };
  void send(int code, const char* content_type = nullptr, const String& content = String("")) 
    { this->_request->send(code, content_type, content); };
//  void sendContent(const String& content)  { this->_request->sendContent(content); };
  void stop()  { this->_request->client()->stop(); };
