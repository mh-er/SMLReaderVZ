// confWeb.cpp - configuration page for WLAN access using AsyncWebServer
//
// 2023-02-17 mh
// - changed "/'>home page" to "/start'>start page"
//
// 2023-01-21 mh
// - derived from IotWebConf.cpp
...
void IotWebConf::handleConfig(WebRequestWrapper* webRequestWrapper)
{
  ...
  if (!dataArrived || !this->validateForm(webRequestWrapper))
  {
    ...
    // -- Add parameters to the form
    this->_systemParameters.renderHtml(dataArrived, webRequestWrapper, content);
    this->_customParameterGroups.renderHtml(dataArrived, webRequestWrapper, content);
    ...
    AsyncWebServerResponse *response = webRequestWrapper->beginResponse(200, "text/html; charset=UTF-8", content);
    response->addHeader("Cache-Control", "no-cache, no-store, must-revalidate");
    response->addHeader("Pragma", "no-cache");
    response->addHeader("Expires", "-1");

    webRequestWrapper->send(response);     
...
//    page += _customHeadElement;
    ...
    else
    {
      ...
      {
        page += F("<p>Your are in WiFi AP mode. Please disconnect to access local WLAN.</p>");
      }
#endif
      page += F("<p>Return to <a href='/start'>start page</a>.</p>");
    }
...
#ifdef IOTWEBCONF_DEBUG_TO_SERIAL
  Serial.print(F("Requested a non-existing page '"));
  Serial.print(webRequestWrapper->url());
  Serial.println("'");
#endif
  String message = "Requested a non-existing page\n\n";
  message += "URI: ";
  message += webRequestWrapper->url();
  message += "\n";
...
bool IotWebConf::handleCaptivePortal(WebRequestWrapper* webRequestWrapper)
{
  ...
  if (!isIp(host) && !host.startsWith(thingName))
  {
    ...
    return true;
#else // IOT_OLD
  String host = webRequestWrapper->host();
  String thingName = String(this->_thingName);
  thingName.toLowerCase();
  if (!isIp(host) && !host.startsWith(thingName))
  {
    ...
    Serial.print(host);
    Serial.print(" redirected to ");
    Serial.print(webRequestWrapper->client()->localIP());
    Serial.print(":");
    Serial.println(webRequestWrapper->client()->localPort());
#endif

// respond with the compressed frontend
    AsyncWebServerResponse *response = webRequestWrapper->beginResponse(302, "text/plain", "");
    
    String s_header = String("http://") + toStringIp(webRequestWrapper->client()->localIP()) + ":" + webRequestWrapper->client()->localPort();
    response->addHeader("Location",s_header);
    webRequestWrapper->send(response);
    return true;
#endif // IOT_OLD
  }
...
void IotWebConf::checkApTimeout()
{
  if ( !mustStayInApMode() )
  {
    ...
    {
      IOTWEBCONF_DEBUG_LINE(F("AP mode Timeout, connecting in STA mode."));
      this->changeState(Connecting);
    }
  }
}
 ...
 * immediately move on.
 * 
 * mh: modified, wait a little to make sure that client login was successful.
 */
